...
function logSection(name: string) {
  ...
  console.log(
    `${colors.cyan}${colors.bright}║${colors.reset}  ${name.padEnd(38)}${colors.cyan}${colors.bright}║${colors.reset}`,
  );
  console.log(
    `${colors.cyan}${colors.bright}╚══════════════════════════════════════════╝${colors.reset}`,
  );
...
function assertWriterResult(result: WriterResult) {
  ...
  assert.equal(result.summary, extractSummary(result.body), "summary 必须从 body 派生");
  assert.equal(result.wordCount, countChineseWords(result.body), "wordCount 必须从 body 派生");
  ...
  result.outline.sections.forEach((section, index) => {
    assert.equal(result.draft[index]?.sectionTitle, section.title, `draft[${index}] 标题不匹配`);
    assert.equal(result.rewrite[index]?.sectionTitle, section.title, `rewrite[${index}] 标题不匹配`);
  });

  result.scores.forEach((score, index) => {
    assert.equal(score.attempt, index + 1, `scores[${index}].attempt 必须连续递增`);
    Object.entries(score.metrics).forEach(([metric, value]) => {
      assert.equal(typeof value, "number", `scores[${index}].metrics.${metric} 必须是数字`);
      assert.ok(value >= 0 && value <= 10, `scores[${index}].metrics.${metric} 超出范围: ${value}`);
    });
  });
...
async function testTrendAgent() {
  ...
  try {
    ...
    log(
      "统计",
      `总源数: ${result.stats.total}, 成功: ${result.stats.success}, 失败: ${result.stats.failed}, 获取: ${result.items.length} 条`,
    );

    console.log(`\n${colors.dim}前 3 条趋势：${colors.reset}`);
    result.items.slice(0, 3).forEach((item, i) => {
      console.log(
        `  ${i + 1}. [${colors.yellow}${item.source}${colors.reset}] ${item.title.slice(0, 60)}...`,
      );
    });
...
async function testTopicAgent(
  trendResult: Awaited<ReturnType<typeof runTrendAgent>>,
  ...
  const startTime = Date.now();
  try {
    const modelConfig = getAgentModelConfig("topic");
    const result = await runTopicAgent(trendResult.items, modelConfig, {
      count: 3,
    });
    const duration = ((Date.now() - startTime) / 1000).toFixed(1);
    ...
    result.topics.forEach((topic, i) => {
      ...
      console.log(
        `  热度: ${"🔥".repeat(Math.ceil(topic.heatScore / 2))} (${topic.heatScore}/10)`,
      );
...
async function testResearchAgent(
  ...
  try {
    ...
    log(
      "提示",
      "Research Agent 需要调用搜索工具，可能需要 2-5 分钟...",
      "yellow",
    );
    ...
    log(
      "统计",
      `关键要点: ${result.keyPoints.length} 条, 来源: ${result.sources.length} 个`,
    );
...
async function testWriterAgent(
  ...
  account: {
    modelConfig: string | Prisma.JsonValue;
    writingStyle?: string | Prisma.JsonValue | null;
  },
) {
  ...
  try {
    ...
    const writingStyle = account.writingStyle
      ? JSON.parse(account.writingStyle as string)
      : undefined;
    ...
    log(
      "统计",
      `标题: ${result.title.slice(0, 40)}..., 字数: ${result.wordCount}, 摘要: ${result.summary.slice(0, 60)}...`,
    );
...
async function testReviewAgent(
  ...
  try {
    ...
    logResult(
      result.passed,
      `审核完成，耗时 ${duration}s，得分: ${result.score}/10`,
    );

    log(
      "维度评分",
      `观点深度: ${result.dimensionScores.perspective}, 结构: ${result.dimensionScores.structure}, 数据支撑: ${result.dimensionScores.dataSupport}, 流畅度: ${result.dimensionScores.fluency}`,
    );
...
${colors.reset}`);
  ...
  try {
    ...
    try {
      ...
      log(
        "跳过",
        "Research Agent 需要真实调用搜索工具，跳过（可手动取消注释测试）",
        "yellow",
      );

      const mockResearch = {
        summary:
          "这是一个模拟的研究摘要，用于测试 Writer Agent。实际研究中会包含详细的数据、案例和专家观点。",
        keyPoints: [
        ...
        rawOutput:
          "完整的研究报告内容，包含详细的数据分析、案例研究和专家观点引用。",
      };

      const writerResult = await testWriterAgent(
        selectedTopic,
        mockResearch as any,
        account,
      );
      ...
      await testReviewAgent(writerResult, account);
      successCount++;
    } catch (error) {
      failCount++;
