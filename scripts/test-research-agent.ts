 ...
 *
 * 用法:
 *   npx tsx scripts/test-research-agent.ts                          # 从最新 topic JSON 加载选题并研究
 *   npx tsx scripts/test-research-agent.ts --topic-index 2          # 研究第3个选题（从0开始）
 *   npx tsx scripts/test-research-agent.ts --from-file <path.json> # 从指定文件加载选题
 *   npx tsx scripts/test-research-agent.ts --fresh                  # 强制重新抓取 Trend+Topic
 *
 * 环境变量:
...
/**
 * 查找最新的 topic JSON 文件（默认行为：自动加载最新）
 */
function findLatestTopicFile(): string | null {
  const topicDir = path.resolve(process.cwd(), "test-output");
  ...
  const files = fs.readdirSync(topicDir)
    ...
    .map((f) => ({
      name: f,
      mtime: fs.statSync(path.join(topicDir, f)).mtime.getTime(),
    }))
    .sort((a, b) => b.mtime - a.mtime);

  return files[0] ? path.join(topicDir, files[0].name) : null;
}
...
function parseArgs(): { topicIndex: number; fromFile: string | null; fresh: boolean } {
  ...
  return { topicIndex, fromFile, fresh };
}
...
async function main() {
  const { topicIndex, fromFile, fresh } = parseArgs();
  loadLocalEnv();

  // 默认行为：自动找最新 topic JSON 文件
  const resolvedFromFile = fresh
    ? null
    : (fromFile ?? findLatestTopicFile());
...
`);
  ...
  let modelConfig;
  try {
    modelConfig = getAgentModelConfig("research");
  } catch (err) {
    console.error(
  ...
  log("选题来源", resolvedFromFile ? `文件: ${path.basename(resolvedFromFile)}` : "强制重新抓取（--fresh）");
  log("选题索引", `${topicIndex}`);
  ...
  if (resolvedFromFile) {
    const resolved = path.resolve(process.cwd(), resolvedFromFile);
    ...
    selectedTopic =
      report.topics[Math.min(topicIndex, report.topics.length - 1)];
    ...
    let topicResult;
    try {
      topicResult = await runTopicAgent(trendResult.items, modelConfig, {
        count: 3,
      });
    } catch (err) {
      console.error(
        `${C.red}Topic Agent 失败: ${err instanceof Error ? err.message : String(err)}${C.r}`,
      );
      process.exit(1);
    }
  ...
  logStep(
    3,
    "运行 Research Agent",
    "深度研究，需要搜索 + 抓取，约需 3-8 分钟...\n",
  );
  log(
    "提示",
    "此过程会执行 10+ 次搜索和 5-6 次网页抓取，请耐心等待...",
    "yellow",
  );
  ...
  try {
    ...
    console.error(
      `${C.red}Research Agent 失败: ${err instanceof Error ? err.message : String(err)}${C.r}`,
    );
    process.exit(1);
  }
  ...
  log(
    "统计",
    `关键要点: ${researchResult.keyPoints.length} 条, 来源: ${researchResult.sources.length} 个`,
  );
  ...
  researchResult.keyPoints.slice(0, 10).forEach((point, i) => {
    console.log(
      `  ${i + 1}. ${point.slice(0, 100)}${point.length > 100 ? "..." : ""}`,
    );
  });
