...
interface TopicOutput {
  ...
  strategy?: TopicSelectionStrategyRaw
  [key: string]: unknown
}
...
interface TopicSuggestionRaw {
  ...
  redSeaLevel?: RedSeaLevelRaw
  contrarianAngle?: string
  timeToMainstream?: TimeToMainstreamRaw
  [key: string]: unknown
}
...
type RedSeaLevelRaw = 'LOW' | 'MEDIUM' | 'HIGH'
type TimeToMainstreamRaw = 'NOW' | 'WEEKS' | 'MONTHS'
type TopicSelectionStrategyRaw = 'VALUE_CREVASSE' | 'EARLY_SIGNAL' | 'CONTRARIAN'
...
const BAD_TITLE_PATTERNS = [/一文读懂/, /深度解读/, /行业分析报告/, /^XX/, /XX行业/]
...
function isRedSeaLevel(value: unknown): value is RedSeaLevelRaw {
  return value === 'LOW' || value === 'MEDIUM' || value === 'HIGH'
}

function isTimeToMainstream(value: unknown): value is TimeToMainstreamRaw {
  return value === 'NOW' || value === 'WEEKS' || value === 'MONTHS'
}

function isTopicSelectionStrategy(value: unknown): value is TopicSelectionStrategyRaw {
  return value === 'VALUE_CREVASSE' || value === 'EARLY_SIGNAL' || value === 'CONTRARIAN'
}
...
export const topicExpert: ExpertSkill = {
  ...
  validateOutput(input: unknown, output: unknown): OutputValidation {
    ...
    if (!isTopicSelectionStrategy(out.strategy)) {
      warnings.push('输出缺少 strategy 字段或值非法')
    }
    ...
    for (let i = 0; i < out.topics.length; i++) {
      ...
      if (!topic.angle || typeof topic.angle !== 'string' || topic.angle.trim() === '') {
        ...
        for (const pattern of BAD_ANGLE_PATTERNS) {
          if (pattern.test(topic.angle)) {
            warnings.push(`topics[${i}].angle 匹配淘汰模式（描述性 angle）：${topic.angle.slice(0, 40)}...`)
            break
          }
        }
      }
      ...
      if (!topic.title || typeof topic.title !== 'string' || topic.title.trim() === '') {
        ...
        for (const pattern of BAD_TITLE_PATTERNS) {
          if (pattern.test(topic.title)) {
            warnings.push(`topics[${i}].title 匹配淘汰模式：${topic.title.slice(0, 40)}...`)
            break
          }
        }
      }
      ...
      if (!summaryStr) {
        ...
        for (const pattern of META_NARRATIVE_PATTERNS) {
          if (pattern.test(summaryStr)) {
            warnings.push(`topics[${i}].summary 包含元叙述（"本文/本篇"等禁止词）：${summaryStr.slice(0, 40)}...`)
            break
          }
        }
      }
      ...
      if (typeof topic.valueScore !== 'number') {
        issues.push(`topics[${i}].valueScore 类型错误（需要 number）`)
      } else if (topic.valueScore < 1 || topic.valueScore > 10) {
        issues.push(`topics[${i}].valueScore 值 ${topic.valueScore} 超出范围 [1, 10]`)
      } else if (topic.valueScore < 5) {
        warnings.push(`topics[${i}].valueScore 为 ${topic.valueScore}（低于 5 分不符合价值洼地标准）`)
      }

      if (!isRedSeaLevel(topic.redSeaLevel)) {
        issues.push(`topics[${i}].redSeaLevel 缺失或非法`)
      }

      if (!topic.contrarianAngle || typeof topic.contrarianAngle !== 'string' || topic.contrarianAngle.trim() === '') {
        issues.push(`topics[${i}].contrarianAngle 缺失或为空`)
      }

      if (!isTimeToMainstream(topic.timeToMainstream)) {
        issues.push(`topics[${i}].timeToMainstream 缺失或非法`)
      }

      if (
        typeof topic.valueScore === 'number' &&
        topic.valueScore >= 8 &&
        topic.redSeaLevel === 'HIGH'
      ) {
        warnings.push(`topics[${i}] valueScore 很高但 redSeaLevel=HIGH，选题逻辑可能不一致`)
      }
    ...
    if (issues.length === 0) {
      const baseScore = 5.5
      const angleQualityBonus =
        (out.topics.filter((t) => {
          const angle = typeof t.angle === 'string' ? t.angle : null
          if (!angle) return false
          return !BAD_ANGLE_PATTERNS.some((p) => p.test(angle))
        }).length /
          Math.max(out.topics.length, 1)) *
        1.5
      const valueScoreBonus =
        out.topics.reduce((acc, t) => acc + (typeof t.valueScore === 'number' ? t.valueScore : 0), 0) /
        Math.max(out.topics.length, 1) /
        10 *
        2
      const heatScoreBonus =
        out.topics.reduce((acc, t) => acc + (typeof t.heatScore === 'number' ? t.heatScore : 0), 0) /
        Math.max(out.topics.length, 1) /
        10 *
        0.5
      const blueOceanBonus =
        (out.topics.filter((t) => t.redSeaLevel === 'LOW').length / Math.max(out.topics.length, 1)) * 0.5
      score = Math.min(10, baseScore + angleQualityBonus + valueScoreBonus + heatScoreBonus + blueOceanBonus)
    }
  ...
  async review(input: unknown, output: unknown): Promise<ExpertResult> {
    ...
    if (out.topics) {
      ...
      const badAngleCount = out.topics.filter((t) => {
        const angle = typeof t.angle === 'string' ? t.angle : null
        return angle ? BAD_ANGLE_PATTERNS.some((p) => p.test(angle)) : false
      }).length
      if (badAngleCount > 0) {
        recommendations.push(`${badAngleCount} 个选题的 angle 属于淘汰类型（描述性/无观点），建议强化 angle 锐度要求`)
      }
      ...
      if (shortAngleCount > 0) {
        recommendations.push(`${shortAngleCount} 个选题的 angle 过短（<15 字），建议补充具体数据或洞察`)
      }

      const metaSummaryCount = out.topics.filter((t) => {
        const summary = typeof t.summary === 'string' ? t.summary : null
        return summary ? META_NARRATIVE_PATTERNS.some((p) => p.test(summary)) : false
      }).length
      if (metaSummaryCount > 0) {
        recommendations.push(`${metaSummaryCount} 个选题的 summary 包含"本文/本篇"等元叙述，应直接描述读者价值`)
      }
      ...
      const avgValueScore =
        ...
        Math.max(out.topics.length, 1)
      if (avgValueScore < 7) {
        recommendations.push(`选题平均 valueScore 仅 ${avgValueScore.toFixed(1)}，建议加强“价值洼地”筛选力度`)
      }
      ...
      const avgHeat =
        ...
        Math.max(out.topics.length, 1)
      if (avgHeat < 5) {
        recommendations.push(`选题平均 heatScore ${avgHeat.toFixed(1)} 分偏低，建议补充更具传播潜力的话题`)
      }

      const highRedSeaCount = out.topics.filter((t) => t.redSeaLevel === 'HIGH').length
      if (highRedSeaCount > 0) {
        recommendations.push(`${highRedSeaCount} 个选题落在红海区，建议优先保留 LOW/MEDIUM 的差异化题目`)
      }
      ...
      if (topicsWithSources < out.topics.length) {
        recommendations.push(`${out.topics.length - topicsWithSources} 个选题缺少引用来源，建议每个选题至少包含 1 个可验证来源`)
      }
      ...
      for (let i = 0; i < out.topics.length; i++) {
        ...
        if (Array.isArray(topic.sources)) {
          ...
          if (validUrls.length === 0) {
            recommendations.push(`topics[${i}] 所有来源 URL 均无效，建议使用可验证的真实链接`)
          }
        }
      }
    }
